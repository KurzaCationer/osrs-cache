...
describe('Browse Type Route', () => {
  ...
  it('renders SpriteCanvas grid for sprites', () => {
    render(<BrowseTypeContent type="sprite" data={[{ id: 100, width: 10 }]} isLoading={false} isError={false} limit={50} offset={0} />)
  ...
  it('renders JsonAssetTable for healthBar', () => {
    render(<BrowseTypeContent type="healthBar" data={[{ id: 1, duration: 50 }]} isLoading={false} isError={false} limit={50} offset={0} />)
    expect(screen.getByTestId('json-asset-table')).toBeDefined()
    expect(screen.queryByTestId('sprite-canvas')).toBeNull()
  })

  it('renders specialized view for dbTable', () => {
    render(<BrowseTypeContent type="dbTable" data={[{ id: 1 }]} isLoading={false} isError={false} limit={50} offset={0} />)
    expect(screen.getByTestId('db-table-browser')).toBeDefined()
  })

  it('renders JsonAssetTable for dbRow with tableId', () => {
    render(<BrowseTypeContent type="dbRow" tableId={1} data={[{ id: 1, values: [] }]} isLoading={false} isError={false} limit={50} offset={0} />)
