...
_\*Encoded indicates the raw data is accessible, and the decoding logic is available in the codebase (aligned with cache2), but the main `getAssets` entry point is not yet utilizing these decoders._
