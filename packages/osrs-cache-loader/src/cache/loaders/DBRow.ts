...
export class DBTable extends PerFileLoadable {
	...
	public static async loadRowIDs(
		...
		let idx = await DBTableIndex.load(cache, table as DBTableID, DBTableIndex.MASTER_COLUMN);
		let vs = idx?.values[0];
		if (!vs) {
			return undefined;
		}
	...
	public static async loadRows(
		...
		return await Promise.all(rows.map(id => DBRow.load(cache, id as any))) as DBRow[];
	}
}
