...
export function decryptXTEA(data: Uint8Array, key: XTEAKey): Uint8Array {
  ...
  const outdv = Reader.makeViewOf(DataView, out)
...
function decryptXTEABlock(
  ...
  for (let r = 0; r < ROUNDS; r++) {
    v1 = ~~(
      v1 -
      ((((v0 << 4) ^ (v0 >>> 5)) + v0) ^ (sum + k[kIndex + ((sum >>> 11) & 3)]))
    )
    sum -= GOLDEN
    v0 = ~~(
      v0 -
      ((((v1 << 4) ^ (v1 >>> 5)) + v1) ^ (sum + k[kIndex + (sum & 3)]))
    )
  }
  out[0] = v0
  out[1] = v1
...
class KeySet {
  ...
  add(key: XTEAKey): boolean {
    if (this.length >= this.data.length * 0.75) {
      this.grow()
    }
    ...
    for (;;) {
      ...
      const d = compareKey(data, index, key)
      if (d == 0) {
        return false
      }
      if (d < 0) {
...
export class XTEAKeyManager {
    ...
    if (Array.isArray(document)) {
      for (const item of document) {
        ...
        if (Array.isArray(item)) {
          ...
          if (this.putKeyForMapsquare(mapsquare as number, key as XTEAKey)) {
            count++
          }
        }
      }
  ...
  public tryDecrypt(ad: ArchiveData, region?: number): Error | undefined {
    ...
    const crypted = ad.getCryptedBlob()
    if (crypted.length < 8) {
    ...
    if (region) {
      const ikeys = this.keysByMapSquare.get(region)
      if (ikeys) {
    ...
    const firstBlock = new Int32Array(2)
    {
      const dv = Reader.makeViewOf(DataView, crypted)
      firstBlock[0] = dv.getInt32(0)
      firstBlock[1] = dv.getInt32(4)
    }
    const decrypted = Int32Array.from(firstBlock)

    const compression = ad.compression
    let error: unknown
    ...
    for (const keyset of keys) {
      ...
      const iterator =
        keyset?.iterator() ?? (() => (hitUndefined++ === 0 ? 0 : -1))
      for (let index: number; (index = iterator()) != -1; ) {
        ...
        if (compression === CompressionType.GZIP) {
          if ((decrypted[1] as unknown) !== 529205248) {
        ...
        try {
          ad.key = data
            ? (Array.from(data.subarray(index, index + 4)) as XTEAKey)
            : undefined
          ad.getDecryptedData()
          return undefined
        } catch (_e) {
