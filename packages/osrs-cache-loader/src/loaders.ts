...
export class ConfigLoader extends AssetLoader {
  ...
  async getCount(): Promise<number> {
    const indexData = await this.cache.getIndex(this.indexId)
    const archRef = indexData?.archives.get(this.archiveId)
    if (!archRef) return 0
    ...
    return files.filter((f) => f.length > 0 && (f.length > 1 || f[0] !== 0))
      .length
  }
  ...
  async getVersion(): Promise<
    { era: 'osrs' | 'rs3'; indexRevision: number } | undefined
  > {
    const indexData = await this.cache.getIndex(this.indexId)
    if (!indexData) return undefined
    return { era: 'osrs', indexRevision: indexData.revision }
  }
}
...
export class ArchiveLoader extends AssetLoader {
  ...
  async getCount(): Promise<number> {
    const indexData = await this.cache.getIndex(this.indexId)
    return await Promise.resolve(indexData?.archives.size ?? 0)
  }
  ...
  async getArchive(archiveId: number): Promise<Uint8Array | undefined> {
    const indexData = await this.cache.getIndex(this.indexId)
    const archRef = indexData?.archives.get(archiveId)
