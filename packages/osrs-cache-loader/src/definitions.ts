import { Reader, CacheVersion } from "./reader";
 ...
 */
export function decodeItem(id: number, data: Uint8Array, version?: CacheVersion): ItemDefinition {
 ...
 */
export function decodeNPC(id: number, data: Uint8Array, version?: CacheVersion): NPCDefinition {
  const reader = new Reader(data, version);
...
export interface ObjectDefinition {
  ...
  isSolid: boolean;
  isInteractable: boolean;
  models: { model: number; shape: number }[];
  modelTypes?: number[];
  recolorFrom?: number[];
  recolorTo?: number[];
  retextureFrom?: number[];
 ...
 */
export function decodeObject(id: number, data: Uint8Array, version?: CacheVersion): ObjectDefinition {
  const reader = new Reader(data, version);
  ...
  while (reader.remaining() > 0) {
    ...
    if (opcode === 1 || opcode === 5) {
      ...
      for (let i = 0; i < count; i++) def.multiAmbientSoundIDs.push(reader.u16());
    } else if (opcode === 81) {
