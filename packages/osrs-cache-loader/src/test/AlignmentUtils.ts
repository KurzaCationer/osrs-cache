...
import { CacheProvider, FileProvider } from "../cache/Cache";
...
export class NodeFSFileProvider implements FileProvider {
	...
	public async getFile(name: string): Promise<Uint8Array | undefined> {
		try {
			return await fs.readFile(path.join(this.path, name));
		} catch (e) {
			return undefined;
		}
	}
	...
	public async exists(name: string): Promise<boolean> {
		try {
			await fs.access(path.join(this.path, name));
			return true;
		} catch (e) {
			return false;
		}
...
export interface AlignmentResult {
  ...
  passed: number;
  failed: number;
  mismatches: Array<{ id: number; expected: any; actual: any; property?: string }>;
}
...
export class AlignmentTester {
   ...
   */
  static compare(name: string, expected: Map<number, any>, actual: Map<number, any>): AlignmentResult {
    ...
    for (const id of ids) {
      ...
      try {
        ...
        if (result.mismatches.length < 10) {
          ...
          result.mismatches.push({ id, expected: exp, actual: act, property, propExpected, propActual });
        }
      }
    }

    console.log(`Alignment [${name}]: ${result.passed}/${result.total} passed (${result.failed} failed)`);
    if (result.failed > 0) {
      console.warn(`Top mismatches for ${name}:`, result.mismatches.slice(0, 3).map(m => ({
        id: m.id,
        property: m.property,
