...
describe("OpenRS2Client", () => {
  ...
  it("should throw OpenRS2Error if no caches found for game", async () => {
    ...
    await expect(client.getLatestCache("oldschool")).rejects.toThrow(OpenRS2Error);
  ...
  it("should fetch XTEA keys", async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith("https://archive.openrs2.org/caches/runescape/123/keys.json");
  });
