...
- You must use `pnpm run dev` for manual verification only if e2e tests are initiated.
...
- **Verification:** Always build the application to verify changes and ensure no compile errors. Use `pnpm run dev` for manual verification during development if e2e tests are initiated. Use `docker compose up --build -d` ONLY when the user needs to provide final feedback on the UI or functionality at the end of a task or phase. Do not use watch modes.
